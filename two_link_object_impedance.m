...
md = 3; kv = 5; kp = 5;
Fimp = @(Xo,dXo,Xd) -kv*(dXo-dXd)-kp*(Xo-Xd);
...
%% command force
Fcmd = @(Xo,dXo,Xd) (pinv(G(Xo))*(Co(dXo)+Mo*md^-1*Fimp(Xo,dXo,Xd)')+(eye(6)-G(Xo)*pinv(G(Xo)))*Fint')'; 
...
for t=0:dt:T
    ...
    for i=1:2
        ...
        dXr = (skew(dXo(4:6))*(SO3.rpy(Xo(4:6))*ro(i,:)'))';
        dqd(i,:) = pinv(J)*(dXo(1:2)'+dXr(1:2)');
        qd(i,:) = qd(i,:)+dt*dqd(i,:);
        ddqd(i,:) = -kv*(dq(i,:)-dqd(i,:))-kp*(q(i,:)-qd(i,:));
        tau(i,:) = rob(i).inertia(q(i,:))*5*ddqd(i,:)'+J'*F(3*i-2:3*i-1)'+...
            rob(i).coriolis(q(i,:),dq(i,:))*dq(i,:)'+rob(i).gravload(q(i,:))';
